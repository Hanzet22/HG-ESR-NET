...
def main():
    ...
    Three ways to pick a model:
      ...
      --model_url           : download a model from a link (Google Drive share
                              links are auto-converted to direct downloads),
                              cached locally, then handled the same as --model_path.
    """
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help=('Download a model from a URL instead of pointing at a local file. Accepts '
              'direct download links (GitHub releases, HuggingFace resolve/ URLs) as well '
              'as Google Drive share links (https://drive.google.com/file/d/<ID>/view...), '
              'which are automatically converted to a direct-download link. Downloads are '
              'cached under weights/from_url/ so re-running with the same --model_url reuses '
              'the cached file. Overrides --model_path and --model_name if given.'))
    parser.add_argument(
        '--arch',
        type=str,
        default=None,
    ...
    parser.add_argument(
        ...
        help=('Logging verbosity for internal diagnostics (arch detection scores, OOM retry '
              'attempts, cache hits/misses, backend selection, etc). Default: info.'))
    parser.add_argument(
        '--verbose', action='store_true',
        help='Shortcut for --log_level debug - show every internal decision, useful for troubleshooting.')

    args = parser.parse_args()

    from realesrgan.logging_setup import setup_logging
    setup_logging(level=args.log_level, verbose=args.verbose)
    logger = logging.getLogger('realesrgan.cli')
    ...
    if args.model_url is not None:
        # URL path: download (or reuse cached download of) any model checkpoint
        # from a link, then hand off to the same universal-loader flow as --model_path.
        from realesrgan.model_url import resolve_model_url, ModelURLError
        ROOT_DIR = os.path.dirname(os.path.abspath(__file__))
        logger.info(f'Resolving --model_url: {args.model_url}')
        try:
            model_path = resolve_model_url(args.model_url, ROOT_DIR)
        except ModelURLError as e:
            logger.error(f'--model_url error: {e}')
            raise SystemExit(1)
        netscale = args.scale
        model = None
        ...
        if not os.path.isfile(model_path):
            ...
            ROOT_DIR = os.path.dirname(os.path.abspath(__file__))
            logger.info(f'Downloading official model "{model_name}"...')
            for url in file_url:
    ...
    # restorer
    logger.debug(f'Building RealESRGANer: model_path={model_path}, scale={netscale}, arch={args.arch}, '
                 f'tile={args.tile}, tile_pad={args.tile_pad}, pre_pad={args.pre_pad}, fp32={args.fp32}')
    upsampler = RealESRGANer(
        ...
        half=not args.fp32,
        gpu_id=args.gpu_id,
        show_tile_progress=(args.log_level != 'debug' and not args.verbose))

    if hasattr(upsampler, '_load_meta'):
        m = upsampler._load_meta
        logger.info(f'Loaded model - backend: {m["backend"]}, arch: {m["arch"]}, scale: {upsampler.scale}, '
                    f'precision: {"fp32" if args.fp32 else "fp16"}')

    if args.face_enhance:  # Use GFPGAN for face enhancement
        from gfpgan import GFPGANer
        logger.info('Loading GFPGAN for --face_enhance...')
    ...
    logger.info(f'Found {len(paths)} file(s) to process in {args.input}')
    ...
    for idx, path in enumerate(paths):
        ...
        if img is None:
            logger.warning(
                f'Skipping {os.path.basename(path)}: not readable as an image by OpenCV '
        ...
        if len(img.shape) == 3 and img.shape[2] == 4:
            ...
            img_mode = None
        if np is not None and img.dtype == np.uint16:
            logger.debug(f'{imgname}: 16-bit input image')
        ...
        try:
            ...
            logger.debug(f'{imgname}: saved to {save_path}')
            n_ok += 1

    logger.info(f'Done: {n_ok} succeeded, {n_skipped} skipped, {n_failed} failed (out of {len(paths)} total)')
