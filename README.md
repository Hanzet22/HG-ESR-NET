...
[![Python](https://img.shields.io/badge/python-3.11%20%7C%203.12%20%7C%203.13-blue.svg)](#-dependencies-and-installation)
...
- **Universal model loader** (`realesrgan/universal_loader.py`) - hybrid
  ...
  (RRDBNet, SRVGGCompact, RealCUGAN 2x/3x/4x) for anything Spandrel doesn't
  recognize.
...
- **`--model_url`** - point at a link instead of a local file. Google Drive
  share links (the most common way OpenModelDB/community checkpoints are
  distributed) are automatically converted to direct downloads and cached
  under `weights/from_url/` for reuse.
- **`.pth` / `.safetensors` / `.onnx` / `.ckpt`** support out of the box.
  ...
  the same detector.
- **Automatic OOM recovery** - a tile that runs out of VRAM during
  processing is automatically retried at a reduced sub-tile size instead of
  corrupting output or crashing the whole run. Memory is also proactively
  freed between images in a batch to reduce fragmentation.
- **`--log_level` / `--verbose`** - structured, timestamped logging you can
  tune from quiet (errors only) to fully verbose (every architecture
  detection score, cache hit, and retry attempt).
- **Python 3.11 / 3.12 / 3.13 ready** - `requirements.txt` and `setup.py`
  updated; `basicsr` is no longer a hard dependency for inference (it's
  still used for the original training pipeline, now an optional
  `pip install .[train]` extra - see [Training](docs/Training.md)).
...
### Using a model from a link (auto-download)

```bash
python inference_realesrgan.py -i inputs/ --model_url "https://drive.google.com/file/d/XXXXX/view?usp=sharing" -o results/
```

Google Drive share links are automatically converted to direct downloads.
The downloaded file is cached under `weights/from_url/`, so re-running with
the same `--model_url` reuses it instead of downloading again.
...
Usage: python inference_realesrgan.py -n MODEL_NAME -i infile -o outfile [options]...
   ...
   or: python inference_realesrgan.py --model_url URL -i infile -o outfile [options]...

  -h                   show this help
  -i --input           Input image or folder. Default: inputs
  -o --output          Output folder. Default: results
  -n --model_name      Official model name (auto-downloaded). Ignored if --model_path/--model_url is set.
  --model_path         Path to any local .pth/.ckpt/.pt/.safetensors checkpoint.
  --model_url           Download a model from a link (Google Drive share links auto-converted). Overrides --model_path/-n.
  ...
  --verbose            Shortcut for --log_level debug.
```

Results are written to the `results/` folder. A tile that runs out of VRAM
is automatically retried at a reduced size rather than failing the whole
image - see [VERSION.md](VERSION.md) for details.
...
RetinexFormer, SCUNet, GRL, DITN, SeemoRe, MoSR, MoESR, DCTLSA, and more -
see [Spandrel's supported architectures](https://github.com/chaiNNer-org/spandrel#supported-architectures)
for the current full list.

Hand-written fallback (`archs2/`, used only if Spandrel doesn't recognize a
checkpoint): RRDBNet, SRVGGCompact, RealCUGAN 2x/3x/4x.
